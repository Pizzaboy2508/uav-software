...
# Drop rows with missing prop size or numeric data
...
for _, row in reference_rows.iterrows():
    ...
    # Estimate throttle for this power
...
for _, row in df.iterrows():
    ...
    plt.plot(powers, thrusts, label=f"{model} (Kv={kv:.0f})")

plt.title("Thrust vs Power (Simulated, Corrected)")
plt.xlabel("Power (W)")
plt.ylabel("Thrust (g)")
plt.grid(True)
plt.legend()
